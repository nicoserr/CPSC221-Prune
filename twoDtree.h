...
private:
   ...
   */
   ...
   void renderNodes(PNG & im, Node * node);

   bool isLeaf(Node * node);

   Node * copyNodes(const Node * orig);

   void pruneNodes(double tol, Node * node);

   bool shouldPrune(double tol, Node * node, HSLAPixel prev_avg);
};
